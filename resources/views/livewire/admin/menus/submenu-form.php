<x-form wire:submit="saveSubmenu({{ $menu->id ?? 'null' }})">
  ...
  <x-choices label="{{ __('Post') }}" wire:model="subPost" :options="$postsSearchable" option-label="title"
    ...
    @change-selection="$wire.changeSelection($event.detail.value)" />
  @elseif($subOption == 2)
  <x-select label="{{ __('Page') }}" option-label="title" :options="$pages" placeholder="{{ __('Select a page') }}"
    wire:model="subPage" wire:change="$refresh" />
  @elseif($subOption == 3)
